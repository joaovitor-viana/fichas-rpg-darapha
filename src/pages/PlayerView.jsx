...
const PlayerView = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-background-dark flex flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className="font-cinzel text-2xl text-slate-400 uppercase tracking-widest">Invocando a Alma...</h2>
    </div>
  );
  ...
  if (error || !player) return (
    <div className="min-h-screen bg-background-dark flex flex-col items-center justify-center p-6 text-center text-slate-100">
      ...
      <h2 className="font-cinzel text-2xl text-slate-400 uppercase tracking-widest">Sussurro das Sombras</h2>
      <p className="text-slate-500 italic mt-4 max-w-md">"Uma barreira impede o Grimório de ler sua alma..."</p>
      <div className="flex gap-4 mt-8">
        <button onClick={fetchPlayerData} className="stone-button px-6 py-2 bg-primary/10 border border-primary/40 text-primary hover:bg-primary/20 transition-all text-xs font-bold uppercase tracking-widest">Tentar Novamente</button>
        <Link to="/dashboard" className="stone-button px-6 py-2 bg-stone/20 border border-slate-700 text-slate-500 hover:text-slate-200 transition-all text-xs font-bold uppercase tracking-widest">Voltar ao Portal</Link>
      </div>
    </div>
  );
    ...
    <div className="bg-background-dark font-display text-slate-200 min-h-screen pb-20 relative overflow-hidden">
      ...
      <header className="flex items-center justify-between border-b border-primary/20 px-6 py-4 lg:px-20 bg-background-dark/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => supabase.auth.signOut()} className="flex items-center justify-center rounded-lg h-10 px-4 bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-all text-sm font-bold tracking-widest uppercase">
            Sair
          </button>
        </div>
      </header>
        ...
        <div className="parchment-texture-dark rounded-xl p-8 md:p-12 flex flex-col gap-12 shadow-2xl relative">
          ...
          <section className="flex flex-col lg:flex-row gap-12 items-center lg:items-end border-b border-slate-800 pb-12">
            <div className="relative group cursor-pointer shrink-0">
              ...
              <label htmlFor="token-upload" className="cursor-pointer block relative">
                <div 
                  className="size-48 rounded-full border-4 border-slate-700 bg-cover bg-center overflow-hidden flex items-center justify-center relative transition-transform hover:scale-105"
            ...
            <div className="flex flex-col text-center lg:text-left flex-1 min-w-0 w-full space-y-4">
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-8">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-widest text-primary font-bold mb-1">Gênero</span>
                  <input className="bg-transparent border-b border-slate-800 focus:border-primary focus:ring-0 p-1 text-slate-100 font-cinzel text-xl w-32" value={player.sexo || ''} onChange={(e) => handleUpdate('sexo', e.target.value)} />
                </div>
              </div>
            </div>
          </section>

          {/* Seção 2: Atributos & Condições */}
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div className="space-y-8">
              <h3 className="font-cinzel text-xl font-bold text-primary flex items-center gap-3 tracking-[0.3em]">
                <span className="material-symbols-outlined">heart_broken</span> SINAIS VITAIS
              </h3>
              <div className="space-y-8">
                {[
                  { label: 'Vida', field: 'vida', color: 'bg-primary' },
                  { label: 'Fome', field: 'fome', color: 'bg-orange-900/80' },
                  { label: 'Sede', field: 'sede', color: 'bg-blue-900/80' }
                ].map((stat) => (
                  <div key={stat.field} className="space-y-3">
                    <div className="flex justify-between items-end">
                      <span className="font-cinzel text-xs uppercase tracking-[0.2em] text-slate-400">{stat.label}</span>
                      <input 
                        type="number"
                    ...
                    <div className="h-2 bg-black/40 rounded-full overflow-hidden border border-slate-800 p-[1px]">
            ...
            <div className="space-y-6">
              <h3 className="font-cinzel text-xl font-bold text-primary flex items-center gap-3 tracking-[0.3em]">
                <span className="material-symbols-outlined">patient_list</span> CONDIÇÕES
              </h3>
              <div className="bg-black/30 border border-slate-800 p-6 rounded-lg min-h-[160px]">
                <textarea 
                  className="w-full bg-transparent border-none focus:ring-0 text-slate-300 font-serif-alt italic leading-relaxed placeholder:text-slate-800"
                  value={player.condicoes ? player.condicoes.join('\n') : ''}
                  onChange={(e) => handleUpdate('condicoes', e.target.value.split('\n'))}
                  placeholder="Ferido, Cansado, Assombrado..."
                />
              </div>
              <p className="text-[10px] text-slate-600 uppercase tracking-widest italic text-right">Uma condição por linha</p>
            </div>
          </section>

          {/* Seção 3: Traços & História */}
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div className="space-y-6">
              <h3 className="font-cinzel text-xl font-bold text-primary flex items-center gap-3 tracking-[0.3em]">
              ...
              <textarea 
                className="w-full bg-black/30 border border-slate-800 rounded-lg p-6 text-slate-300 font-serif-alt italic leading-relaxed focus:ring-1 focus:ring-primary min-h-[140px] placeholder:text-slate-800"
                value={player.conviccao || ''}
                onChange={(e) => handleUpdate('conviccao', e.target.value)}
                placeholder="Qual o propósito da sua alma neste plano?"
              />
            </div>
            <div className="space-y-6">
              <h3 className="font-cinzel text-xl font-bold text-primary flex items-center gap-3 tracking-[0.3em]">
              ...
              <textarea 
                className="w-full bg-black/30 border border-slate-800 rounded-lg p-6 text-slate-300 leading-relaxed focus:ring-1 focus:ring-primary min-h-[140px] placeholder:text-slate-800"
                value={player.caracteristicas || ''}
                onChange={(e) => handleUpdate('caracteristicas', e.target.value)}
                placeholder="Traços de personalidade, aparência, segredos..."
              />
            </div>
          </section>

          {/* Seção 4: Posses & Equipamentos */}
          <section className="space-y-12">
            <h3 className="font-cinzel text-xl font-bold text-primary flex items-center gap-3 tracking-[0.3em] border-b border-slate-800 pb-4">
              <span className="material-symbols-outlined">inventory_2</span> POSSES & EQUIPAMENTOS
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="space-y-8">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-widest text-primary font-bold mb-2">Arma Principal</span>
                  <div className="flex items-center gap-4 bg-black/40 p-4 rounded-lg border border-slate-800 group focus-within:border-primary/50 transition-colors">
                    ...
                    <input 
                      ...
                      placeholder="Espada de Prata, Clava, etc."
                    />
                  </div>
                </div>

                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-widest text-primary font-bold mb-2">Veículo / Montaria</span>
                  <div className="flex items-center gap-4 bg-black/40 p-4 rounded-lg border border-slate-800 group focus-within:border-primary/50 transition-colors">
                    ...
                    <input 
                      ...
                      placeholder="Carruagem, Corcel, etc."
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col">
                <span className="text-[10px] uppercase tracking-widest text-primary font-bold mb-2">Inventário Completo</span>
                <div className="bg-black/30 border border-slate-800 p-6 rounded-lg flex-1 min-h-[160px]">
                  <textarea 
                    className="w-full h-full bg-transparent border-none focus:ring-0 text-slate-300 leading-relaxed placeholder:text-slate-800"
          ...
          <section className="space-y-6">
            <h3 className="font-cinzel text-xl font-bold text-red-900 flex items-center gap-3 tracking-[0.3em]">
              <span className="material-symbols-outlined">warning</span> TORMENTOS & TRAUMAS
            </h3>
            <div className="bg-red-950/10 border border-red-900/30 p-8 rounded-lg">
              <textarea 
                className="w-full bg-transparent border-none focus:ring-0 text-red-200/70 font-serif-alt italic leading-relaxed min-h-[120px] placeholder:text-slate-900"
                value={player.tormentos ? player.tormentos.join('\n') : ''}
                onChange={(e) => handleUpdate('tormentos', e.target.value.split('\n'))}
                placeholder="Pavor do fogo, Lembranças da guerra, Vozes..."
              />
            </div>
          </section>
